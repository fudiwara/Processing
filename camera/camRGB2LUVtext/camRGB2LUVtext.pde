...
void draw(){
  ...
  if(mousePressed){
    int x = constrain(mouseX, 0, w - 1);
    int y = constrain(mouseY, 0, h - 1);
    noFill();
    stroke(255, 0, 0);
    strokeWeight(3);
    ellipse(x, h + y, 7, 7);
    ...
    fill(255, 0, 0);
    int l = (int)brightness(lImg.pixels[x + y * w]);
    int u = (int)brightness(uImg.pixels[x + y * w]);
    int v = (int)brightness(vImg.pixels[x + y * w]);
    text(l + ", " + u + ", " + v, 20, 20);
  }
}
